...
def test_launch_route_opens_browser(monkeypatch):
    ...
    sonic_app.PERPETUAL_TOKENS = {'SOL': {'id': 1}}
    popen_args = []
    monkeypatch.setattr(subprocess, 'Popen', lambda args: popen_args.append(args))

    result = sonic_app.launch_jupiter_position('Sonic - R2', 'SOL', 'long')
    assert result == 'Launching SOL in Sonic - R2'
    assert popen_args == [[
        'C:/Program Files/Google/Chrome/Application/chrome.exe',
        '--profile-directory=Sonic - R2',
        'https://jup.ag/perps/long/SOL-SOL',
    ]]
