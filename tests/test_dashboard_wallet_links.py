import types
import pytest
from flask import DummyFlask as Flask
...
def _setup_app(monkeypatch, context):
    ...
    import app.dashboard_bp as dashboard_bp_mod
    monkeypatch.setattr(
        dashboard_bp_mod,
        "get_dashboard_context",
        lambda dl, sc: context,
...
def test_dash_page_handles_missing_wallet(monkeypatch):
    ...
    with app.test_client() as client:
        ...
        assert "/launch/Default/BTC/long" in html
...
def test_dash_page_uses_wallet_profile(monkeypatch):
    ...
    with app.test_client() as client:
        ...
        assert resp.status_code == 200
        html = resp.data.decode()
        assert "/launch/Profile1/ETH/long" in html
